...
.side{border-right:1px solid var(--line);padding:16px;background:rgba(8,10,16,.86);backdrop-filter:blur(16px);display:flex;flex-direction:column;gap:12px;height:100dvh;max-height:100dvh;min-height:0;position:sticky;top:0;overflow:hidden}
...
.main{min-width:0;height:100dvh;max-height:100dvh;min-height:0;display:flex;flex-direction:column;overflow:hidden}.top{flex:0 0 auto;height:64px;z-index:5;border-bottom:1px solid var(--line);background:rgba(7,8,13,.74);backdrop-filter:blur(16px);display:flex;align-items:center;padding:0 20px;gap:8px}.top strong{font-size:15px}.top span{margin-left:auto;color:var(--muted);font-size:12px;white-space:nowrap;overflow:hidden;text-overflow:ellipsis;min-width:0}.installTop,.memoryTop,.conversationTop,.modelToggle{border:1px solid #654a8a;border-radius:10px;background:#1d1728;color:var(--txt);padding:7px 10px;cursor:pointer;white-space:nowrap;flex:0 0 auto}.memoryTop{border-color:#3f4d67;background:#151b27}.conversationTop{display:none;border-color:#3f4d67;background:#151b27}.modelToggle{border-color:#3f4d67;background:#151b27}.modelToggle[aria-pressed="true"]{border-color:#8b5cf6;background:#241b38;color:#e9d5ff}.mobileState{display:none}
.chat{width:min(920px,100%);margin:0 auto;display:flex;flex-direction:column;flex:1;min-height:0;overflow:hidden;padding:18px 18px calc(18px + env(safe-area-inset-bottom))}.messages{flex:1;min-height:0;overflow-y:auto;overflow-x:hidden;overscroll-behavior:contain;-webkit-overflow-scrolling:touch;padding:28px 4px 18px;display:flex;flex-direction:column;gap:12px;scrollbar-width:thin}.empty{margin:auto;text-align:center;color:var(--muted);max-width:460px}.empty .orb{width:56px;height:56px;border-radius:18px;margin:0 auto 13px;display:grid;place-items:center;background:linear-gradient(145deg,#241a34,#171322);border:1px solid #44335d;color:var(--accent);font-size:24px}.msg{max-width:min(78%,720px);white-space:pre-wrap;word-break:break-word;border:1px solid var(--line);padding:11px 13px;box-shadow:0 12px 36px rgba(0,0,0,.2)}.msg.user{align-self:flex-end;background:linear-gradient(145deg,#3b2858,#291d42);border-radius:16px 16px 5px 16px}.msg.assistant{align-self:flex-start;background:var(--panel);border-radius:16px 16px 16px 5px}.role{font-size:10px;text-transform:uppercase;letter-spacing:.09em;color:var(--muted);font-weight:800;margin-bottom:5px;display:flex;align-items:center;gap:8px}.msgTime{margin-left:auto;font-size:10px;font-weight:600;letter-spacing:.02em;color:var(--muted);opacity:.8;text-transform:none}.messageActions{display:flex;gap:6px;margin-top:9px;flex-wrap:wrap}.messageAction{display:inline-flex;align-items:center;gap:6px}.messageAction svg{width:13px;height:13px;flex:0 0 auto}.messageBody p{margin:.45em 0}.messageBody p:first-child{margin-top:0}.messageBody p:last-child{margin-bottom:0}.messageBody pre{overflow:auto;background:#090c12;border:1px solid var(--line);border-radius:10px;padding:10px;white-space:pre}.messageBody code{font-family:ui-monospace,SFMono-Regular,Consolas,monospace}.messageBody a{color:var(--accent)}.messageActions{display:flex;gap:6px;margin-top:9px}.messageAction{border:1px solid #343b4d;border-radius:8px;background:#151925;color:var(--muted);padding:5px 8px;font-size:11px;cursor:pointer}.messageAction:hover{color:var(--txt);border-color:#5a6682}
.composer{flex:0 0 auto;background:rgba(7,8,13,.88);backdrop-filter:blur(16px);border:1px solid var(--line);border-radius:18px;padding:10px;box-shadow:0 -14px 44px rgba(0,0,0,.3)}.iconBtn{display:inline-grid;place-items:center}.iconBtn svg{width:20px;height:20px}.send.iconBtn{display:inline-flex;align-items:center;justify-content:center;gap:8px}.stopGeneration{white-space:nowrap}.composeRow{display:grid;grid-template-columns:auto minmax(0,1fr) auto;gap:8px;align-items:end}.composer textarea{width:100%;min-height:64px;max-height:180px;resize:vertical;background:#0a0d14;color:var(--txt);border:1px solid var(--line);border-radius:13px;padding:12px;outline:none}.composer textarea:focus{border-color:#6e54a0;box-shadow:0 0 0 3px rgba(139,92,246,.1)}.send,.mic{height:44px;border-radius:12px;color:white;cursor:pointer}.send{padding:0 18px;border:1px solid #7e5cb2;background:linear-gradient(180deg,#7650aa,#583b82)}.mic{width:48px;border:1px solid #4c5263;background:#171b27;font-size:19px}.mic.recording{border-color:#d66c7a;background:#431f27;box-shadow:0 0 0 3px rgba(255,139,154,.12)}.send:disabled,.mic:disabled{opacity:.55;cursor:not-allowed}.composer small{display:block;color:var(--muted);padding:8px 2px 0}
...
<main class="main">
  ...
  <div class="chat">
    ...
    <section class="composer"><div class="composeRow"><button id="mic" class="mic iconBtn" type="button" aria-label="Record microphone" aria-pressed="false"><svg viewBox="0 0 24 24" width="20" height="20" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true"><rect x="9" y="2" width="6" height="12" rx="3"/><path d="M5 10a7 7 0 0 0 14 0M12 19v3"/></svg></button><textarea id="message" maxlength="8000" placeholder="Message MesoAI…" rows="2"></textarea><button id="send" class="send iconBtn" type="button" aria-label="Send message"><svg viewBox="0 0 24 24" width="20" height="20" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true"><path d="M12 19V5M5 12l7-7 7 7"/></svg><span class="sendLabel">Send</span></button></div><small>Private · Local STT + Meso voice</small></section>
  </div>
</main>
</div>
