...
(() => {
  ...
  const baseStatus=()=> 'Private · Persona meso-v1 · Meso voice · Local XTTS';
  const voiceLabel=(profile)=>profile==='meso-v2.2'?'Meso Voice v2.2':profile==='meso-v2'?'Meso Voice v2':'Meso voice';
  function setReady(button,note,profile=''){button.textContent='▶ Play';button.disabled=false;button.setAttribute('aria-pressed','false');button.title='Play prepared Meso voice reply';note.textContent=` ${voiceLabel(profile)} ready`;}
  function setIdle(button=activeButton,note=activeNote){if(button){if(button._mesoXttsUrl)setReady(button,note||{textContent:''},button._mesoProfile||'');else{button.textContent='▶ Play';button.disabled=false;button.setAttribute('aria-pressed','false');}}activeButton=null;activeNote=null;activeAudio=null;if(status)status.textContent=baseStatus();}
  function stopPlayback(){if(activeAbort){try{activeAbort.abort();}catch(_){}activeAbort=null;}if(activeAudio){try{activeAudio.pause();activeAudio.currentTime=0;}catch(_){}}if(synth&&(synth.speaking||synth.pending))synth.cancel();setIdle();}
  function containsArabic(text){return /[\u0600-\u06ff\u0750-\u077f\u08a0-\u08ff]/.test(text);}
  function browserFallback(text,button,note){if(!synth||typeof window.SpeechSynthesisUtterance!=='function'){setIdle(button,note);note.textContent=' Voice unavailable';return;}const u=new SpeechSynthesisUtterance(text);u.lang=containsArabic(text)?'ar-AE':'en-US';u.rate=.96;activeButton=button;activeNote=note;button.textContent='■ Stop';button.setAttribute('aria-pressed','true');note.textContent=' Browser fallback';u.addEventListener('end',()=>setIdle(button,note),{once:true});u.addEventListener('error',()=>setIdle(button,note),{once:true});synth.speak(u);}
  function createPreparedAudio(url){const a=new Audio();a.preload='auto';a.playsInline=true;a.muted=false;a.volume=1;a.src=url;try{a.load();}catch(_){}return a;}
  function clearPrepared(button){if(!button)return;if(button._mesoXttsAudio){try{button._mesoXttsAudio.pause();}catch(_){}}button._mesoXttsAudio=null;button._mesoXttsUrl='';button._mesoProfile='';}
  ...
  async function handlePlay(text,button,note){const clean=String(text||'').trim();if(!clean)return;if(activeButton===button&&activeAudio&&!activeAudio.paused){stopPlayback();return;}if(button._mesoXttsUrl){playPreparedXtts(button,note);return;}const prepared=await prepareXtts(clean,button,note,{background:false});if(!prepared)browserFallback(clean,button,note);}
  function decorate(card,autoPrepare=false){if(!(card instanceof HTMLElement)||!card.classList.contains('assistant')||!card.classList.contains('msg')||card.dataset.replyAudioReady==='1')return;card.dataset.replyAudioReady='1';const body=card.children.length>=2?card.children[1]:null;const text=body?String(body.textContent||'').trim():'';if(!text||/^Chat error:/i.test(text)||/^Microphone .*error:/i.test(text))return;const tools=document.createElement('div');tools.style.marginTop='9px';const button=document.createElement('button');button.type='button';button.textContent='▶ Play';button.setAttribute('aria-label','Play assistant reply using Meso voice');Object.assign(button.style,{padding:'6px 10px',borderRadius:'9px',border:'1px solid #4c5263',background:'#171b27',color:'#f6f7fb',cursor:'pointer',fontSize:'12px'});const note=document.createElement('span');note.textContent=autoPrepare?' Preparing · Meso voice':' Meso voice';Object.assign(note.style,{marginLeft:'7px',color:'#9299aa',fontSize:'10px'});button.addEventListener('click',()=>handlePlay(text,button,note));tools.append(button,note);card.appendChild(tools);if(autoPrepare)queueMicrotask(()=>prepareXtts(text,button,note,{background:true}));}
  function decorateAll(root=messages,autoPrepare=false){if(root instanceof HTMLElement&&root.matches('.msg.assistant'))decorate(root,autoPrepare);if(root.querySelectorAll)for(const card of root.querySelectorAll('.msg.assistant'))decorate(card,autoPrepare);}
  for(const row of document.querySelectorAll('.side .status')){const label=row.querySelector('span:first-child'),pill=row.querySelector('.pill');if(label&&pill&&String(label.textContent||'').trim()==='Cloned voice'){pill.textContent='MESO VOICE';pill.classList.add('good');}}
  decorateAll();new MutationObserver(records=>{for(const record of records)for(const node of record.addedNodes)if(node instanceof HTMLElement)decorateAll(node,true);}).observe(messages,{childList:true,subtree:true});
  window.addEventListener('pagehide',stopPlayback);window.addEventListener('beforeunload',stopPlayback);
})();
