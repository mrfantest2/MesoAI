...
(() => {
  ...
  async function transcribeBlob(blob){transcribing=true;setBusy(true,'Transcribing locally…');try{const response=await fetch('/meso/api/transcribe.php',{method:'POST',credentials:'same-origin',cache:'no-store',headers:{'Content-Type':blob.type||'audio/webm','Accept':'application/json'},body:blob});let body={};try{body=await response.json();}catch(_){body={ok:false,error:'invalid_json'};}if(response.status===403){location.reload();return;}if(!response.ok||!body.ok||!String(body.transcript||'').trim())throw new Error(body.error||`HTTP ${response.status}`);input.value=String(body.transcript).trim();transcribing=false;setBusy(false,`Transcribed locally${body.language?` · ${body.language}`:''}`);await sendText();}catch(error){addMessage('assistant',`Microphone transcription error: ${error.message}`,'local STT');}finally{transcribing=false;if(!send.disabled)status.textContent=baseStatus();if(mic)mic.disabled=false;}}
