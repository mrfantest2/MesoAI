...
if ($method !== 'GET' && $method !== 'HEAD') {
    http_response_code(405); header('Allow: GET, HEAD'); header('Content-Type: application/json; charset=utf-8');
...
if (is_file($metaPath) && is_readable($metaPath)) {
    $mediaMeta = json_decode((string)file_get_contents($metaPath), true);
    if (!is_array($mediaMeta)
        || ($mediaMeta['engine'] ?? '') !== 'xtts-v2'
        || ($mediaMeta['format'] ?? '') !== 'mp3'
        || !in_array((string)($mediaMeta['profile'] ?? ''), ['meso-a','meso-v2','meso-v2.2'], true)) {
        http_response_code(404); exit;
    }
    $voiceProfile = (string)$mediaMeta['profile'];
    $voiceLanguage = in_array((string)($mediaMeta['language'] ?? ''), ['ar','en'], true) ? (string)$mediaMeta['language'] : '';
}
...
if($voiceLanguage!=='')header('X-Meso-Voice-Language: '.$voiceLanguage);
if($method==='HEAD')exit;
