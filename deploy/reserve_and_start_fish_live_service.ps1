...
'@
    ...
    do {
        ...
        if ($response.errors) { throw ('RunPod GraphQL error: ' + ($response.errors | ConvertTo-Json -Compress)) }
        $eligible = @()
        foreach ($g in @($response.data.gpuTypes)) {
            if ($g.secureCloud -ne $true -or [int]$g.memoryInGb -lt 24) { continue }
            if ($null -eq $g.lowestPrice -or $null -eq $g.lowestPrice.uninterruptablePrice) { continue }
            $price = [decimal]$g.lowestPrice.uninterruptablePrice
            if ($price -le 0 -or $price -gt $MaxHourlyUsd) { continue }
            $stock = [string]$g.lowestPrice.stockStatus
            $counts = @($g.lowestPrice.availableGpuCounts | ForEach-Object { [int]$_ })
    ...
    if ($null -eq $pod -or [string]::IsNullOrWhiteSpace([string]$pod.id)) { throw 'RunPod did not return a Pod ID.' }
    $podId = [string]$pod.id
    ...
    do {
        ...
        if ($state.machine -and $state.machine.PSObject.Properties['secureCloud'] -and $state.machine.secureCloud -ne $true) { throw 'RunPod did not confirm Secure Cloud.' }
        if ($null -ne $state.adjustedCostPerHr -and [string]$state.adjustedCostPerHr -ne '') { $cost = [decimal]$state.adjustedCostPerHr }
        elseif ($null -ne $state.costPerHr -and [string]$state.costPerHr -ne '') { $cost = [decimal]$state.costPerHr }
        if ($null -ne $cost -and $cost -gt $MaxHourlyUsd) { throw "Actual RunPod price exceeds cap: $cost/hour" }
        $ip = [string]$state.publicIp
        if ($state.portMappings -and $state.portMappings.PSObject.Properties['22']) { $sshPort = [string]$state.portMappings.'22' }
        Write-Host "MESO_FISH_LIVE_POD_STATUS=$($state.desiredStatus) COST=$cost SSH_READY=$([bool]($ip -and $sshPort))"
        if ($state.desiredStatus -eq 'RUNNING' -and $null -ne $cost -and $ip -and $sshPort) { break }
    } while ((Get-Date) -lt $readyDeadline)
    if ($state.desiredStatus -ne 'RUNNING' -or $null -eq $cost -or !$ip -or !$sshPort) { throw 'Reserved Pod did not become price-verified and SSH-ready.' }
